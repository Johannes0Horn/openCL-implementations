...
OpenCLMgr::~OpenCLMgr()
{
	...
	//Release kernels
	if (hashtableinsert_kernel) status = clReleaseKernel(hashtableinsert_kernel);
	if (hashtableread_kernel) status = clReleaseKernel(hashtableread_kernel);
...
cl_int OpenCLMgr::init()
{
	...
	CHECK_SUCCESS("Error: creating command queue")
		...
		const char *filename = "HashTable_Kernel.cl";
	string sourceStr;
	...
	hashtableinsert_kernel = clCreateKernel(program, "hashtableinsert_kernel", &status);
	CHECK_SUCCESS("Error: creating hashtableinsert_kernel")
		hashtableread_kernel = clCreateKernel(program, "hashtableread_kernel", &status);
	CHECK_SUCCESS("Error: creating hashtableread_kernel")
