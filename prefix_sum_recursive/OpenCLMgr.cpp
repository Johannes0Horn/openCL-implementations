...
OpenCLMgr::OpenCLMgr()
{
	...
	praefixsumme512_kernel = 0;
	praefixsumme_E_kernel = 0;

	praefixsumme_kernel = 0;
...
OpenCLMgr::~OpenCLMgr()
{
	...
	if (praefixsumme512_kernel) status = clReleaseKernel(praefixsumme512_kernel);
	if (praefixsumme_E_kernel) status = clReleaseKernel(praefixsumme_E_kernel);


	if (praefixsumme_kernel) status = clReleaseKernel(praefixsumme_kernel);
...
cl_int OpenCLMgr::init()
{
	...
	CHECK_SUCCESS("Error: creating command queue")
		...
		const char *filename = "PraefixSumme_Kernel.cl";
	string sourceStr;
	...
	CHECK_SUCCESS("Error: creating praefixsumme256_kernel")
		praefixsumme512_kernel = clCreateKernel(program, "praefixsumme512_kernel", &status);
	CHECK_SUCCESS("Error: creating praefixsumme_kernel")
		praefixsumme_kernel = clCreateKernel(program, "praefixsumme_kernel", &status);
	CHECK_SUCCESS("Error: creating praefixsumme_kernel")
		praefixsumme_E_kernel = clCreateKernel(program, "praefixsumme_E_kernel", &status);
	CHECK_SUCCESS("Error: creating praefixsumme_kernel")
